...
fn default_context_task_template() -> String {
    "帮助用户创作和完善角色设定, 需要从多个角度(角色动机，角色心理，角色性格，角色背景)等分析，完成角色卡。当用户要求(帮忙填写)的时候，主动使用工具(edit_character)填写用户要求的 field。当用户确认添加 worldbook 条目的时候，使用工具(create_world_book_entry)。当用户明确要求删除某个世界书条目时，优先使用 delete_world_book_entry，并尽量传 entry_id 以避免误删。".to_string()
}

fn default_context_instructions_template() -> String {
    "基于用户需求分析现有角色设定，提供建议并调用相应工具。\n始终保持角色设定的一致性和逻辑性，遵循用户的具体要求。\n如果需要修改角色信息，请使用 edit_character 工具。\n如果需要添加世界书条目，请使用 create_worldbook_entry 工具。".to_string()
}
...
impl AIConfigService {
    ...
    fn default_character_assistant_role() -> AIRole {
        AIRole {
            ...
            context_task_template: "帮助用户创作和完善角色设定, 需要从多个角度(角色动机，角色心理，角色性格，角色背景)等分析，完成角色卡。当用户要求(帮忙填写)的时候，主动使用工具(edit_character)填写用户要求的 field。当用户确认添加 worldbook 条目的时候，使用工具(create_world_book_entry)。当用户明确要求删除某个世界书条目时，优先使用 delete_world_book_entry，并尽量传 entry_id 以避免误删。".to_string(),
